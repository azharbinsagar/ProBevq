...
require_once("../asset/db.php");
      ...
      // code for Activate users    
        if(isset($_GET['hid']))
        {
          $hid = $_GET['hid']; // id of user to be activated

          $q2 = mysqli_query($con, "UPDATE `homedelivery` SET `status` = '1' WHERE `homedelivery`.`hid` = '$hid'") or die("database error:". mysqli_error($con));        
         {
      ...
      <div class="card-body py-3 p-0">
        <div class="row cwp23-content">
						...
						<div class="table table-responsive">
							...
							<thead> <tr> <th>S.No</th> <th>Product Name</th><th>Name</th>  <th>Phone Number</th> <th>Address</th><th>District</th><th>Pincode</th><th>Action</th></tr> </thead>
							<tbody> 
						

								<?php 
								//taking data from database
										$query = "SELECT * FROM `homedelivery` where `sid` = '$sid' and `status`= '0'  ORDER BY `name`";
                            ...
                            <td>$obj->pincode</td>";
                            if($obj->status == 0) 
                              { echo "<td><a href=\"hdelivery.php?hid=$obj->hid\" onclick=\"return confirm('Are you sure you want to Deliver this product?');\"><button class=\"btn btn-danger\">Accept Order</button></a></td>";}
                            else
